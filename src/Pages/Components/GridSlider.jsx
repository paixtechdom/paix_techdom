import { useContext, useEffect, useState } from "react"
import { AppContext } from "../../App"

export const GridSlider = ({data}) =>{

return(
        <div className="flex flex-col items-center mt-9 overflow-hidden">
    
                  
        <div  className={`grid grid-cols-1 lg:grid-cols-2 w-11/12 lg:w-9/12 gap-9`}>

            {
                data.map((d, i) => (
                    <Parallax key={i} id={d.id} className='flex flex-col border border-blue-900 p-9 rounded-xl gap-2  heading-relaxed tracking-wide' index={i}>
                        
                        <div className="flex relative w-fi center h-16 w-3/12 p-3 mb-3">
                            <div className="absolute border border-blue-900 top-0 left-50 w-16 h-16 rotate-45 rounded-tl-[25px]  rounded-br-[25px]">

                            </div>
                            <i className={`bi bi-${d.icon}  text-4xl text-blue-400`}></i>
                        </div>

                        <p className="text-blue-600">{d.title}</p>
                        <p className="text-sm text-gray-200">{d.desc}</p>
                    </Parallax>
                ))
            }

        </div>
                  
        </div>

    )
}


const Parallax = ({id, children, className, index}) => {
    const [ isPosMatch, setIsPosMatch ] = useState(false)

    const handleScroll = () => {
        const element = document.querySelector(`#${id}`)
        if(element){
            let pos = element.getBoundingClientRect()
            if(pos.top > -10 && pos.top < 900){
                setIsPosMatch(true)
            } else{
                setIsPosMatch(false)
            }

        }
      
    }

    useEffect(() =>{
        document.addEventListener('scroll', handleScroll)
        
    }, [])

    return(
        <div id={id} className={`relative `}>
            <div className={`relative  transition-all duration-[1s] ${!isPosMatch ? `${index == 0 || index == 2 || index == 4 || index == 6 ? '-translate-x-[100vw]' : 'translate-x-[100vw]'} ` : 'translate-x-[0px]'} ${className}`}>
                {children}
            </div>
        </div>
    )
}